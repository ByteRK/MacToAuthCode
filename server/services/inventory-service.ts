...
export class InventoryService {
  ...
  private confirmPassword(password:string){if(password!==this.adminPassword)throw new Error('管理员密码不正确')}
  ...
  update(id:number,input:CodeInput,password:string){this.confirmPassword(password);return this.repo.database.transaction(()=>{const before=this.repo.get(id);if(!before)throw new Error('授权记录不存在');const record=this.repo.update(id,this.normalize(input));this.audit.record({action:'updated',entityId:id,pid:record?.pid,did:record?.did,message:'编辑授权码',snapshot:{before,after:record}});return record})}
