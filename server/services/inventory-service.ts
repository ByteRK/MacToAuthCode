...
export class InventoryService {
  constructor(private repo:AuthCodeRepository,private audit:AuditService,private adminPassword:string){}
  ...
  update(id:number,input:CodeInput){return this.repo.database.transaction(()=>{const before=this.repo.get(id);if(!before)throw new Error('授权记录不存在');const record=this.repo.update(id,this.normalize(input));this.audit.record({action:'updated',entityId:id,pid:record?.pid,did:record?.did,message:'编辑授权码',snapshot:{before,after:record}});return record})}
  unbind(id:number){return this.repo.database.transaction(()=>{const before=this.repo.get(id);if(!before)throw new Error('授权记录不存在');const record=this.repo.unbind(id);this.audit.record({action:'unbound',entityId:id,pid:before.pid,mac:before.assignedMac,did:before.did,message:'解除设备绑定',snapshot:before});return record})}
  delete(ids:number[],password:string){if(password!==this.adminPassword)throw new Error('管理员密码不正确');return this.repo.database.transaction(()=>{let deleted=0;for(const id of ids){const record=this.repo.get(id);if(!record)continue;this.repo.delete(id);deleted++;this.audit.record({action:'deleted',entityId:id,pid:record.pid,mac:record.assignedMac,did:record.did,message:'删除授权码',snapshot:record})}return {deleted}})}
}
