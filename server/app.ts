...
export async function buildApp(config:AppConfig,database=new Database(config.databasePath)) {
  ...
  app.put('/api/admin/codes/:id',async(request,reply)=>{try{return {success:true,data:inventory.update(Number((request.params as {id:string}).id),request.body as never)}}catch(error){return apiError(reply,error)}})
  app.post('/api/admin/codes/:id/unbind',async(request,reply)=>{try{return {success:true,data:inventory.unbind(Number((request.params as {id:string}).id))}}catch(error){return apiError(reply,error)}})
  ...
  app.post('/api/admin/import',async(request,reply)=>{try{const {buffer,fields}=await uploaded(request);const result=excel.import(buffer,fields.defaultPid??fields.default_pid??'');return result.ok?{success:true,data:result}:reply.code(400).send({success:false,message:'导入校验失败',errors:result.errors})}catch(error){return apiError(reply,error)}})
...
function apiError(reply:FastifyReply,error:unknown){const message=(error as Error).message;const conflict=message.includes('UNIQUE constraint failed');return reply.code(conflict?409:400).send({success:false,message:conflict?'同一 PID 下的 DID 不能重复':message})}
