...
import {Box,ChartNoAxesCombined,FileClock,FileInput,DatabaseZap,LogOut,Tags,Send,CloudDownload} from 'lucide-vue-next'
import {useRoute,useRouter} from 'vue-router'
import{ElMessage}from'element-plus'
import {api} from '../api/client'
const route=useRoute(),router=useRouter()
const items=[['/','运行总览',ChartNoAxesCombined],['/pids','PID 清单',Tags],['/inventory','授权码管理',Box],['/import','批量导入导出',FileInput],['/ciot-import','CIOT 源导入',CloudDownload],['/allocations','分配记录',Send],['/logs','操作与请求记录',FileClock],['/migration','旧数据库迁移',DatabaseZap]] as const
