...
<template>
  <PageHeader title="批量导入导出" description="通过标准 Excel 模板维护授权库存，并导出分配记录">
    <el-button :icon="Download" tag="a" href="/api/admin/export/template">下载模板</el-button>
    <el-button type="primary" :icon="Download" tag="a" href="/api/admin/export/assigned">导出已分配记录</el-button>
  </PageHeader>
  <ContentCard title="导入授权库存" description="识别 pid、did、license；其余列将保存到自定义载荷">
    <div class="import-layout">
      <div>
        <FileDropzone accept=".xlsx" hint="仅支持 .xlsx 文件，最大 50 MB" @change="file = $event" />
        <el-form label-position="top">
          <div class="form-grid">
            <el-form-item label="默认 PID">
              <el-input v-model="defaultPid" placeholder="仅在文件没有 pid 列时使用" />
            </el-form-item>
            <el-form-item label="导入批次">
              <el-input v-model="sourceBatch" placeholder="留空则自动生成，如 20260826_1243" />
            </el-form-item>
          </div>
          <el-button type="primary" :icon="Upload" :loading="loading" :disabled="!file"
      ...
      <div class="result-panel">
        <template v-if="result?.ok">
          ...
          <p>批次：{{ result.sourceBatch }}<br>共 {{ result.totalRows }} 行，跳过 {{ result.skipped }} 条重复数据</p>
        </template>
  ...
  <el-dialog v-model="duplicateDialog" title="发现重复授权码" width="760" :close-on-click-modal="false">
    ...
    <el-table class="duplicate-table" :data="preview?.duplicates ?? []" max-height="360">
      <el-table-column label="序号" width="70" align="center">
        <template #default="{ $index }">{{ $index + 1 }}</template>
      </el-table-column>
      <el-table-column prop="row" label="Excel 行" width="90" />
      <el-table-column prop="pid" label="PID" min-width="160" />
      <el-table-column prop="did" label="DID" min-width="190" />
