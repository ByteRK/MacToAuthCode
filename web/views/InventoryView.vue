...
<template>
  <PageHeader title="授权码管理" description="维护库存、已分配关系及完整授权载荷">
    ...
    <el-button type="primary" :icon="Plus" @click="openEdit()">新增授权码</el-button>
  </PageHeader>
  <ContentCard>
    <div v-if="query.pid" class="active-filter">
      <span>当前仅展示 PID：</span>
      <strong>{{ query.pid }}</strong>
      <el-button link type="primary" @click="clearPidFilter">清除筛选</el-button>
    </div>
    <div class="toolbar">
      <el-input v-model="query.search" clearable placeholder="搜索 DID / MAC / 载荷" :prefix-icon="Search"
        @keyup.enter="applyFilters" />
      <el-select v-model="query.status" @change="applyFilters">
        <el-option label="全部状态" value="all" />
        <el-option label="可分配" value="available" />
        <el-option label="已分配" value="assigned" />
      </el-select>
      <el-button @click="applyFilters">查询</el-button>
      <el-button type="danger" plain :disabled="!deleteIds.length" @click="openDeleteDialog">删除所选</el-button>
    </div>
    <el-table v-loading="loading" :data="items" row-key="id" @selection-change="selected = $event">
      ...
      <el-table-column prop="did" label="DID" min-width="170" />
      <el-table-column label="状态" width="100">
        <template #default="{ row }">
          <el-tag :type="row.status === 'assigned' ? 'success' : 'info'">{{ row.status === 'assigned' ? '已分配' : '可分配'
            }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="assignedMac" label="绑定 MAC" min-width="160">
        <template #default="{ row }">{{ row.assignedMac || '-' }}</template>
      </el-table-column>
    ...
    <div class="pagination">
      <el-pagination v-model:current-page="query.page" v-model:page-size="query.pageSize"
        ...
        @change="refreshWithState" />
    </div>
  </ContentCard>
  <el-dialog v-model="dialog" :title="editing ? '编辑授权码' : '新增授权码'" width="620">
    <el-form label-position="top">
      <div class="form-grid">
        <el-form-item label="PID">
          <el-input v-model="form.pid" :class="{ fieldChanged: fieldChanged('pid') }" />
        </el-form-item>
        <el-form-item label="DID">
          <el-input v-model="form.did" :class="{ fieldChanged: fieldChanged('did') }" />
        </el-form-item>
        <el-form-item label="License">
          <el-input v-model="form.license" :class="{ fieldChanged: fieldChanged('license') }" />
        </el-form-item>
        <el-form-item label="来源批次">
          <el-input v-model="form.sourceBatch" :class="{ fieldChanged: fieldChanged('sourceBatch') }" />
        </el-form-item>
      </div>
      <el-form-item label="完整载荷 JSON">
        <el-input v-model="form.payloadText" :class="{ fieldChanged: fieldChanged('payloadText') }" type="textarea"
          :rows="8" />
      </el-form-item>
      <el-alert v-if="hasChanges" class="change-hint" title="高亮字段已修改，可点击重置按钮恢复原始数据" type="warning" :closable="false"
        show-icon />
      <el-form-item v-if="editing" label="请输入授权码操作密码进行二次确认">
        <el-input v-model="editPassword" type="password" show-password @keyup.enter="save" />
      </el-form-item>
    </el-form>
    <template #footer>
      <el-button :icon="X" @click="dialog = false">取消</el-button>
      <el-button v-if="editing" :icon="RotateCcw" :disabled="!hasChanges" @click="resetForm">重置</el-button>
      <el-button type="primary" :icon="Save" :disabled="!!editing && (!editPassword || !hasChanges)"
        @click="save">保存</el-button>
    </template>
  </el-dialog>
  <el-dialog v-model="unbindDialog" title="确认解除设备绑定" width="460">
    <p class="dialog-warning">解除后，该授权码会恢复为可分配状态，原设备再次请求时可能获得其它授权码</p>
    <el-form label-position="top">
      <el-form-item label="请输入授权码操作密码进行二次确认">
        <el-input v-model="unbindPassword" type="password" show-password @keyup.enter="unbind" />
      </el-form-item>
    </el-form>
    <template #footer>
      <el-button @click="unbindDialog = false">取消</el-button>
      <el-button type="primary" :disabled="!unbindPassword" @click="unbind">确认解除绑定</el-button>
    </template>
  </el-dialog>
  <el-dialog v-model="deleteDialog" title="确认删除授权码" width="460">
    <p class="dialog-warning">将删除 {{ pendingDeleteIds.length }} 条记录，包括其中的已分配关系。此操作会写入审计日志</p>
    <el-form label-position="top" @submit.prevent="remove">
      <el-form-item label="请输入授权码操作密码进行二次确认">
        <el-input v-model="deletePassword" type="password" show-password @keydown.enter.prevent.stop="remove" />
      </el-form-item>
    </el-form>
    <template #footer>
      <el-button @click="deleteDialog = false">取消</el-button>
      <el-button type="danger" :loading="deleting" :disabled="!deletePassword || !pendingDeleteIds.length"
        @click="remove">确认删除</el-button>
    </template>
  </el-dialog>
</template>
<style scoped>
...
.fieldChanged:deep(.el-input__wrapper) {
  background: #fff9ed;
  box-shadow: 0 0 0 1px #c59652 inset
}

.fieldChanged:deep(.el-textarea__inner) {
  background: #fff9ed;
  box-shadow: 0 0 0 1px #c59652 inset
}
</style>
