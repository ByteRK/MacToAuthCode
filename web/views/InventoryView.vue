...
<template>
  ...
  <el-dialog v-model="dialog" :title="editing?'编辑授权码':'新增授权码'" width="620"><el-form label-position="top"><div class="form-grid"><el-form-item label="PID"><el-input v-model="form.pid" :class="{fieldChanged:fieldChanged('pid')}"/></el-form-item><el-form-item label="DID"><el-input v-model="form.did" :class="{fieldChanged:fieldChanged('did')}"/></el-form-item><el-form-item label="License"><el-input v-model="form.license" :class="{fieldChanged:fieldChanged('license')}"/></el-form-item><el-form-item label="来源批次"><el-input v-model="form.sourceBatch" :class="{fieldChanged:fieldChanged('sourceBatch')}"/></el-form-item></div><el-form-item label="完整载荷 JSON"><el-input v-model="form.payloadText" :class="{fieldChanged:fieldChanged('payloadText')}" type="textarea" :rows="8"/></el-form-item><el-alert v-if="hasChanges" class="change-hint" title="高亮字段已修改，可点击重置按钮恢复原始数据" type="warning" :closable="false" show-icon/><el-form-item v-if="editing" label="请输入授权码操作密码进行二次确认"><el-input v-model="editPassword" type="password" show-password @keyup.enter="save"/></el-form-item></el-form><template #footer><el-button :icon="X" @click="dialog=false">取消</el-button><el-button v-if="editing" :icon="RotateCcw" :disabled="!hasChanges" @click="resetForm">重置</el-button><el-button type="primary" :icon="Save" :disabled="!!editing&&(!editPassword||!hasChanges)" @click="save">保存</el-button></template></el-dialog>
  <el-dialog v-model="unbindDialog" title="确认解除设备绑定" width="460"><p class="dialog-warning">解除后，该授权码会恢复为可分配状态，原设备再次请求时可能获得其它授权码。</p><el-form label-position="top"><el-form-item label="请输入授权码操作密码进行二次确认"><el-input v-model="unbindPassword" type="password" show-password @keyup.enter="unbind"/></el-form-item></el-form><template #footer><el-button @click="unbindDialog=false">取消</el-button><el-button type="primary" :disabled="!unbindPassword" @click="unbind">确认解除绑定</el-button></template></el-dialog>
  <el-dialog v-model="deleteDialog" title="确认删除授权码" width="460"><p class="dialog-warning">将删除 {{pendingDeleteIds.length}} 条记录，包括其中的已分配关系。此操作会写入审计日志。</p><el-form label-position="top" @submit.prevent="remove"><el-form-item label="请输入授权码操作密码进行二次确认"><el-input v-model="deletePassword" type="password" show-password @keydown.enter.prevent.stop="remove"/></el-form-item></el-form><template #footer><el-button @click="deleteDialog=false">取消</el-button><el-button type="danger" :loading="deleting" :disabled="!deletePassword||!pendingDeleteIds.length" @click="remove">确认删除</el-button></template></el-dialog>
</template>
<style scoped>
