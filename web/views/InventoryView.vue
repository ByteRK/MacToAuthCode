...
const dialog=ref(false),editing=ref<number|null>(null),editPassword=ref(''),deleteDialog=ref(false),deletePassword=ref(''),unbindDialog=ref(false),unbindTarget=ref<AuthCodeRecord|null>(null),unbindPassword=ref('')
...
function openEdit(row?:AuthCodeRecord){editing.value=row?.id??null;editPassword.value='';Object.assign(form,row?{pid:row.pid,did:row.did,license:row.license,sourceBatch:row.sourceBatch??'',payloadText:JSON.stringify(row.payload,null,2)}:{pid:'',did:'',license:'',sourceBatch:'',payloadText:'{}'});dialog.value=true}
async function save(){try{const body={...form,payload:JSON.parse(form.payloadText),...(editing.value?{password:editPassword.value}:{})};await api(editing.value?`/api/admin/codes/${editing.value}`:'/api/admin/codes',json(editing.value?'PUT':'POST',body));ElMessage.success('保存成功');dialog.value=false;editPassword.value='';load()}catch(e){ElMessage.error((e as Error).message)}}
function confirmUnbind(row:AuthCodeRecord){unbindTarget.value=row;unbindPassword.value='';unbindDialog.value=true}
async function unbind(){if(!unbindTarget.value)return;try{await api(`/api/admin/codes/${unbindTarget.value.id}/unbind`,json('POST',{password:unbindPassword.value}));ElMessage.success('已解除绑定');unbindDialog.value=false;unbindTarget.value=null;unbindPassword.value='';load()}catch(e){ElMessage.error((e as Error).message)}}
...
<template>
  ...
  <el-dialog v-model="dialog" :title="editing?'编辑授权码':'新增授权码'" width="620"><el-form label-position="top"><div class="form-grid"><el-form-item label="PID"><el-input v-model="form.pid"/></el-form-item><el-form-item label="DID"><el-input v-model="form.did"/></el-form-item><el-form-item label="License"><el-input v-model="form.license"/></el-form-item><el-form-item label="来源批次"><el-input v-model="form.sourceBatch"/></el-form-item></div><el-form-item label="完整载荷 JSON"><el-input v-model="form.payloadText" type="textarea" :rows="8"/></el-form-item><el-form-item v-if="editing" label="请输入管理员密码进行二次确认"><el-input v-model="editPassword" type="password" show-password @keyup.enter="save"/></el-form-item></el-form><template #footer><el-button @click="dialog=false">取消</el-button><el-button type="primary" :disabled="!!editing&&!editPassword" @click="save">保存</el-button></template></el-dialog>
