...
<template><PageHeader title="运行总览" description="快速了解授权库存、产品覆盖和设备分发状态。"><el-button :icon="RefreshCw" :loading="loading" @click="load">刷新数据</el-button></PageHeader><div class="metric-grid"><ContentCard v-for="[label,key,Icon] in cards" :key="key"><div class="metric"><div class="metric-icon"><component :is="Icon"/></div><div><span>{{label}}</span><strong>{{summary[key]??0}}</strong></div></div></ContentCard></div><ContentCard title="服务说明" description="设备可通过局域网 HTTP 接口领取与 PID 对应的授权数据。"><div class="endpoint"><code>POST /api/device/authorize</code><span>请求字段：mac、pid</span></div></ContentCard></template>
