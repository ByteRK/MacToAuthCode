...
- PID 产品备注使用独立数据表管理，支持 JSON 文件导入导出。
- 支持多个 PID 同时进行持久化生产计数，实时统计本轮首次成功分发数量。
- Excel模板、整表校验导入、分配记录和审计日志导出。
- CIOT 源文件可纯转换为常规批量导入使用的三列 Excel，不影响数据库和审计日志。
- 审计日志在主库保留最近 90 天，超期记录按月自动归档，并可在后台直接选择已有归档数据库查看。
