...
- 运行总览。
- 生产计数：可同时对多个 PID 启动独立计数，启动时必须填写目标数量和生产备注；仅在首次成功分配授权码时递增，重复请求不计数，达到目标后继续累计并显示超出数量。计数和运行状态保存在 `production_counters`，软件重启后不丢失。任务只提供“取消计数”操作，二次确认后删除任务及全部进度；启动和取消的审计说明均保留操作发生时的实时数量、目标数量及备注。独立页面每 2 秒刷新，运行总览同步展示正在计数的 PID 摘要。
- PID 清单：按 PID 不区分大小写升序排列；点击产品备注即可行内编辑；库存 `>10` 为绿色、`1-10` 为黄色、`0` 为红色；可输入 MAC 直接调用设备接口申请授权码；“查看数据”跳转到 `/inventory?pid=...` 并自动筛选。清单只展示存在授权码数据的 PID。产品备注与授权码数据隔离，支持使用 `[{"pid":"...","remark":"..."}]` JSON 增量导入和完整导出；导入导出只以 `pid_metadata` 为备注数据源，提前导入的备注会在对应 PID 出现授权码后显示。
...
- 审计日志自动归档：主数据库仅保留最近 90 天；程序启动时检查一次，之后每 24 小时检查一次。历史日志按月份写入 `data/audit-archives/audit-YYYY-MM.db`，确认归档写入完整后才删除主库记录。操作与请求记录页会自动扫描并列出已有归档数据库，冷数据只读查询。
- 旧数据库迁移。
