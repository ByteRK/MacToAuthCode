...
describe('PID management',()=>{
  ...
  it('filters audit records by multiple actions',async()=>{await app.inject({method:'PUT',url:'/api/admin/pids/P1001/remark',headers:{cookie},payload:{remark:'产品'}});const result=(await app.inject({method:'GET',url:'/api/admin/logs?action=created,updated',headers:{cookie}})).json().data;expect(new Set(result.items.map((item:{action:string})=>item.action))).toEqual(new Set(['created','updated']))})
})
