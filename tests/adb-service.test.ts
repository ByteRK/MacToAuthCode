...
describe('ADB authorization writer',()=>{
  ...
  it('reads MAC, allocates normally, replaces slots, pushes and syncs',async()=>{repo.create({pid:'P1',did:'D1',license:'L1',payload:{}});const runner=new FakeRunner(args=>args.includes('cat')?{stdout:'aa:bb:cc:11:22:33\n',stderr:''}:{stdout:args.includes('push')?'1 file pushed':'',stderr:''}),service=new AdbService(new DistributionService(repo,audit),audit,runner);const result=await service.write({serial:'SERIAL-1',pid:'P1',networkInterface:'wlan0',targetPath:'/sdcard/license.txt',template:'{{pid}}|{{did}}|{{license}}'});expect(result).toMatchObject({pid:'P1',mac:'AA:BB:CC:11:22:33',did:'D1',license:'L1',content:'P1|D1|L1'});expect(runner.calls.at(-2)).toMatchObject(['-s','SERIAL-1','push',expect.any(String),'/sdcard/license.txt']);expect(runner.calls.at(-1)).toEqual(['-s','SERIAL-1','shell','sync']);expect(audit.list(10,'adb_write_succeeded')).toHaveLength(1)})
  ...
  it('reboots the selected device without creating an audit record',async()=>{const runner=new FakeRunner(()=>({stdout:'',stderr:''})),service=new AdbService(new DistributionService(repo,audit),audit,runner);expect(await service.reboot('SERIAL-2')).toEqual({serial:'SERIAL-2',rebooting:true});expect(runner.calls).toEqual([['-s','SERIAL-2','reboot']]);expect(audit.list(10)).toHaveLength(0)})
})
