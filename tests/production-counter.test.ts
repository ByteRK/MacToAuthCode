...
describe('production counters',()=>{const dirs:string[]=[];afterEach(()=>{for(const dir of dirs.splice(0))rmSync(dir,{recursive:true,force:true})});it('counts only first allocations, persists across restarts and supports cancellation',()=>{
  const dir=mkdtempSync(join(tmpdir(),'production-counter-test-'));dirs.push(dir);const path=join(dir,'db.sqlite')
  let database=new Database(path),repo=new AuthCodeRepository(database),audit=new AuditService(database),counters=new ProductionCounterService(database,audit),distribution=new DistributionService(repo,audit,counters)
  ...
  counters.cancel('P1');expect(counters.list().some(item=>item.pid==='P1')).toBe(false);expect(audit.list(1,'production_counter_cancelled')[0].message).toContain('实时数量：1；目标数量：1；备注：白班');database.close()
})})
